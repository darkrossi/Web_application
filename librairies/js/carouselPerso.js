...
/*
 * HTML 5 Shiv - respond.js doesn't support height
...
(function (e, t) {
    ...
    function d(e, n, r) {
        ...
        return o.canHaveChildren && !i.test(e) ? r.frag.appendChild(o) : o
    }
    function v(e, n) {
        if (!e) {
            e = t
    ...
    function g(e) {
        ...
        if (y.shivCSS && !o && !n.hasCSS) {
            n.hasCSS = !!c(e, "article,aside,figcaption,figure,footer,header,hgroup,nav,section{display:block}" + "mark{background:#FF0;color:#000}")
        }
        if (!l) {
            m(e, n)
        }
    ...
    var y = {elements: r.elements || "abbr article aside audio bdi canvas data datalist details figcaption figure footer header hgroup mark meter nav output progress section summary time video", version: n, shivCSS: r.shivCSS !== false, supportsUnknownElements: l, shivMethods: r.shivMethods !== false, type: "default", shivDocument: g, createElement: d, createDocumentFragment: v};
    e.html5 = y;
    g(t)
})(this, document)

/*
 * Respond.js with min height adjustment
 * https://github.com/scottjehl/Respond/pull/96/files
 */
...
(function (e) {
    ...
    var t = e.document, n = t.documentElement, r = [], i = [], s = [], o = {}, u = 30, a = t.getElementsByTagName("head")[0] || n, f = a.getElementsByTagName("link"), l = [], c = function () {
        var t = f, n = t.length, r = 0, i, s, u, a;
        for (; r < n; r++) {
            i = t[r], s = i.href, u = i.media, a = i.rel && i.rel.toLowerCase() === "stylesheet";
            if (!!s && a && !o[s]) {
                if (i.styleSheet && i.styleSheet.rawCssText) {
                    p(i.styleSheet.rawCssText, s, u);
                    o[s] = true
                } else {
                    if (!/^([a-zA-Z]+?:(\/\/)?)/.test(s) || s.replace(RegExp.$1, "").split("/")[0] === e.location.host) {
                        l.push({href: s, media: u})
                    }
                }
            }
        }
        ...
        for (; f < o; f++) {
            ...
            for (; l < d; l++) {
                h = p[l];
                r.push({media: h.match(/(only\s+)?([a-zA-Z]+)(\sand)?/) && RegExp.$2, rules: i.length - 1, minw: h.match(/\(min\-width:[\s]*([\s]*[0-9\.]+)(px|em)[\s]*\)/) && parseFloat(RegExp.$1) + (RegExp.$2 || ""), maxw: h.match(/\(max\-width:[\s]*([\s]*[0-9\.]+)(px|em)[\s]*\)/) && parseFloat(RegExp.$1) + (RegExp.$2 || ""), minh: h.match(/\(min\-height:[\s]*([\s]*[0-9\.]+)(px|em)[\s]*\)/) && parseFloat(RegExp.$1) + (RegExp.$2 || ""), maxh: h.match(/\(max\-height:[\s]*([\s]*[0-9\.]+)(px|em)[\s]*\)/) && parseFloat(RegExp.$1) + (RegExp.$2 || "")})
            }
        }
        y()
    }, d, v, m = function () {
        ...
        var o = "clientWidth", l = n[o], c = t.compatMode === "CSS1Compat" && l || t.body[o] || l, h = "clientHeight", l = n[h], p = t.compatMode === "CSS1Compat" && l || t.body[h] || l, b = {}, w = f[f.length - 1], E = (new Date).getTime();
        if (e && d && E - d < u) {
            clearTimeout(v);
            v = setTimeout(y, u);
            return
        ...
        for (var S in r) {
            var x = r[S], T = x.minw, N = x.maxw, C = x.minh, k = x.maxh, L = "em";
            if (!!T) {
                T = parseFloat(T) * (T.indexOf(L) > -1 ? g || m() : 1)
            }
            if (!!N) {
                N = parseFloat(N) * (N.indexOf(L) > -1 ? g || m() : 1)
            }
            if (!!C) {
                C = parseFloat(C) * (C.indexOf(L) > -1 ? g || m() : 1)
            }
            if (!!k) {
                k = parseFloat(k) * (k.indexOf(L) > -1 ? g || m() : 1)
            }
            if ((!T && !N || (!T || T && c >= T) && (!N || N && c <= N)) && (!C && !k || (!C || C && p >= C) && (!k || k && p <= k))) {
                if (!b[x.media]) {
                    b[x.media] = []
                }
                b[x.media].push(i[x.rules])
            }
        }
        for (var S in s) {
            if (s[S] && s[S].parentNode === a) {
                a.removeChild(s[S])
            }
        }
        for (var S in b) {
            var A = t.createElement("style"), O = b[S].join("\n");
            A.type = "text/css";
            A.media = S;
            a.insertBefore(A, w.nextSibling);
            if (A.styleSheet) {
    ...
    if (e.addEventListener) {
        e.addEventListener("resize", E, false)
    } else if (e.attachEvent) {
        e.attachEvent("onresize", E)
    }
})(this);
 ...
 * Adjustment made by Shaun Morrison (me): https://github.com/woothemes/FlexSlider/pull/392
 */
(function (e) {
        ...
        p = {init: function () {
                ...
                setTimeout(function () {
                    ...
                    r.slides.click(function (t) {
                        ...
                        var u = n.offset().left - e(r).scrollLeft();
                        if (u <= 0 && n.hasClass(s + "active-slide")) {
                            r.flexAnimate(r.getTarget("prev"), true)
                        } else if (!e(i.asNavFor).data("flexslider").animating && !n.hasClass("active")) {
                            r.direction = r.currentItem < o ? "next" : "prev";
                            r.flexAnimate(o, i.pauseOnAction, false, true, true)
                        }
                    })
            ...
            if (!c) {
                ...
                if (t === "init") {
                    if (!o) {
                        ...
                        r.slides.css({opacity: 0, display: "block", webkitTransition: "opacity " + i.animationSpeed / 1e3 + "s ease", zIndex: 1}).eq(r.currentSlide).css({opacity: 1, zIndex: 2})
                    }
                }
                if (i.smoothHeight)
                    p.smoothHeight()
...
$(function () {
    ...
    //Add classes - usually first and last for grid layout
    $('#slide-nav li:last').addClass('last');

    //add form classes (thanks IE6)
    $('input[type="text"]').addClass('inp-text');


    //Page gallery
    var $slideshow = '#slideshow';
    var $slideshowWrap = $($slideshow);
    var $slideshowNav = '#slide-nav3';
    var $slideshowNavWrap = $($slideshowNav);

    $slideshowNavWrap.flexslider({
        animation: 'slide',
        controlNav: false,
        animationLoop: false,
        directionNav: false,
        ...
        asNavFor: $slideshow,
        start: function () {
            $($slideshowNav + ' img').load(function () {
                setSlideNavItemHeight();
                slideNavFix();
                centralSlideNav();
            });
        }
    ...
    $('.slideshow-next').click(function (e) {
        e.preventDefault();
        $slideshowWrap.flexslider('next');
    });

    $('.slideshow-prev').click(function (e) {
        e.preventDefault();
        $slideshowWrap.flexslider('prev');
    });
    ...
    //Hide intro text on scroll
    var animated = false;
    var $intro = $('#intro');
    function hideIntro() {
        //Test if already animated or hidden in this session

        if (animated == false && $.cookie('cookieIntro') != 'hidden') {
            $intro
                    .css({
                        'overflow': 'hidden'
                    })
    ...
    //Don't show intro if seen already in session

    if ($.cookie('cookieIntro') == 'hidden') {
        $intro.hide();
    }
    ...
    //determine whether to fix slide-nav or not
    function slideNavFix() {
        var $el = $('#slide-nav');
        if ($($el).length) { //check if el exists otherwise IE messes up.
            var $elWrapOffset = $('#slide-nav-wrap').offset().top;
            var $elHt = $el.height();
            var $winScroll = $(window).scrollTop();
            var $winHt = $(window).height();
            var $elPos = ($elWrapOffset + $elHt);
            var $winPos = ($winScroll + $winHt)

            // give parent container same height to save space when scrolls back into place
            $el.parent().height($elHt);
            if ($winPos >= $elPos) {
                $el
                        .removeClass('fixed')
                        .css({
    ...
    //set active class for nav on slide transition
    function setSlideControlActive(slider) {
        var $nav = $('.slideshow-control-sml');
        var $navNext = $('.slideshow-next');
        var $navPrev = $('.slideshow-prev');
    ...
    $('.slideshow-control-sml').click(function () {
        $(this).removeClass('active');
    });

    //set height for small slide controls.
    function setSlideControlSml() {
        $($slideshow + ' .flex-active-slide').each(function () {
            var $el = $(this);
            var $itemHt = $el.find('.media img').height();
            $('.slideshow-control-sml')
                    .animate({
                        top: $itemHt + 15
                    })
        });
    ...
    //Centralise slide thumbs when bigger than max width. Also prevents flexslider bug
    function centralSlideNav() {
        var $winWid = $(window).width();
        var $item = $($slideshowNav + ' li');
        var $itemWid = $item.first().width();
        var $itemCount = $item.size();
        var $itemsTotalWid = ($itemCount * 15) + ($itemWid * $itemCount);
        if ($winWid >= $itemsTotalWid) {
            $slideshowNavWrap.css({
                'width': $itemsTotalWid,
                'margin-left': 'auto',
                'margin-right': 'auto'
            });
    ...
    //set the height for each carousel item. Need for vertically aligning text
    function setSlideNavItemHeight() {
        $($slideshowNav + ' li').each(function () {
            var $el = $(this);
            var $itemHt = $el.height();
            $el.find('h3').css({
                'height': $itemHt
            });
        });
    ...
    //close cookie notice
    $('#cookie-notice .close a').click(function (e) {
        e.preventDefault();
        $(this).closest('#cookie-notice').slideUp();
        $.cookie('cookieNoticeState', 'hidden', {expires: 60});
    });
    //console.log($.cookie('cookieNoticeState'));
    if ($.cookie('cookieNoticeState') == 'hidden') {
        $('#cookie-notice').hide();
    }

    //Detect if mobile and add class
...
if (window.location.host != 'localhost') {
    ...
    _gaq.push(['_setAccount', 'UA-36333623-1']);
